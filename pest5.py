...
def cadastrar_carro():
    ...
    # Se o ano for igual ao que estamos, criar chave com conteúdo "Novo", se não for igual será adicionado em "Usado" e ainda criará a chave km rodados e pedirá ao usuário os km rodados que o carro tem)
    p = 1
    while p == 1:
        dados_car.clear()
        ...
        if pergunta == 1:
            ...
            if mod_carro == dados_car['Modelo PCD']:
...
def senha_user():
    while True:
        ...
        pass_user= str(pass_user)
        print(pass_user)
        for valor in dados_user:
            if pass_user == valor['Senha']:
                print('Login Ok!!')
                if ((nome_user == usuarios['Usuário'] and pass_user == valor['Senha']) and valor['Comp/Vend'] == 'V') == True :  #((user and s) and c/v)==True
                    menu_vendedor()
                    break
                elif ((nome_user == usuarios['Usuário'] and pass_user == valor['Senha']) and valor['Comp/Vend'] == 'C') == True :
                    menu_comprador()
                    teste['Login']=1
                    break    
                break
